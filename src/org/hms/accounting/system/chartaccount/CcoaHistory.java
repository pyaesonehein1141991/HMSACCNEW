...
@Entity
@Table(name = TableName.CCOA_HISTORY)
@TableGenerator(name = "CCOA_HISTORY_GEN", table = "ID_GEN", pkColumnName = "GEN_NAME", valueColumnName = "GEN_VAL", pkColumnValue = "CCOA_HISTORY_GEN", allocationSize = 10)
@EntityListeners(IDInterceptor.class)
public class CcoaHistory implements Serializable {
	...
	@Id
	@GeneratedValue(strategy = GenerationType.TABLE, generator = "CCOA_HISTORY_GEN")
	private String id;
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		CcoaHistory other = (CcoaHistory) obj;
		if (acName == null) {
	...
	public CcoaHistory(CurrencyChartOfAccount ccoa) {
		super();
		this.id = ccoa.getId();
		this.coa = ccoa.getCoa();
		this.acName = ccoa.getAcName();
		this.branch = ccoa.getBranch();
		this.department = ccoa.getDepartment();
		this.currency = ccoa.getCurrency();
		this.bF = ccoa.getbF();
		this.oBal = ccoa.getoBal();
		this.hOBal = ccoa.gethOBal();
		this.cBal = ccoa.getcBal();
		this.monthlyRate = ccoa.getMonthlyRate();
		this.msrcRate = ccoa.getMsrcRate();
		this.bfRate = ccoa.getBfRate();
		this.bfsrcRate = ccoa.getBfsrcRate();
		this.mrevRate = ccoa.getMrevRate();
		this.lymsrcRate = ccoa.getLymsrcRate();
		this.sccrBal = ccoa.getSccrBal();
		this.accrued = ccoa.getAccrued();
		this.budget = ccoa.getBudget();
		this.version = ccoa.getVersion();
		this.basicEntity = ccoa.getBasicEntity();
	}
}
